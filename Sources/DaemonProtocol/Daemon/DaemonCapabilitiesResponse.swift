...
//
// DaemonCapabilitiesResponse: shape of the `daemon.capabilities`
// RPC reply.
//
...
// from the PROVENANCE-CHECKED connection (`DispatchPeerContext.authenticatedSession`),
// never a payload cap. A stolen cap in the body must not surface a victim's
// role/grant advertising. `role: nil` means the connection isn't authenticated
// (out-of-tab); the daemon falls back to the daemon-wide subset of
// `allowedMethods` rather than rejecting, so out-of-tab `deviceterm --help`
