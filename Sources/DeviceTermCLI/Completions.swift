...
public enum Completions {
    ...
    private static func bashScript() -> String {
        ...
        _deviceterm() {
            ...
            verbs="\(verbs)"
            flags="--duration --hold --velocity --step --tab --pane --window --mode --to-tab --type-delay --all --json"
    ...
    private static func fishScript() -> String {
        ...
        // Help topics.
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from help' "
            + "-a '\(helpTopics.joined(separator: " "))'"
        )
        // Sub-verbs.
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from tabs' "
            + "-a '\(subVerbList("tabs"))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from panes' "
            + "-a '\(subVerbList("panes"))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from devices' "
            + "-a '\(subVerbList("devices"))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from device' "
            + "-a '\(deviceSubVerbs.joined(separator: " "))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from ax' "
            + "-a '\(subVerbList("ax"))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from completions' "
            + "-a '\(subVerbList("completions"))'"
        )
        lines.append(
            "complete -c deviceterm -n "
            + "'__fish_seen_subcommand_from completions; "
            + "and __fish_seen_subcommand_from install' "
            + "-a 'zsh bash fish'"
        )
        // Workspace sub-verbs.
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from tab' "
            + "-a '\(tabSubVerbs.joined(separator: " "))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from pane' "
            + "-a '\(paneSubVerbs.joined(separator: " "))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from window' "
            + "-a '\(windowSubVerbs.joined(separator: " "))'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from windows' "
        ...
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from button' "
            + "-a '\(buttonsList)'"
        )
        lines.append(
            "complete -c deviceterm -n '__fish_seen_subcommand_from rotate' "
            + "-a '\(rotateList)'"
        )
        // Flags.
