...
// item (an iPhone glyph plus a count) belongs to the daemon's faceless
// helper. A window capture of the *app* therefore never contains the
// status item, which is the daemon's own (menu-bar-layer) window,
// captured on its own via the status-item path. The harness never
