...
/// A pane id together with the admission it was observed under, naming which
/// admission of a pane a close is talking about. Nil `attachment` comes from
/// a daemon predating the field, which closes unconditionally.
///
...
///
/// The daemon enforces the same pairing from its side: `pane.closeById`
/// carrying `expecting` is refused when the record has moved on. This type is
/// the GUI half, so a close the user was asked about is fenced before it is
/// sent as well as when it arrives.
