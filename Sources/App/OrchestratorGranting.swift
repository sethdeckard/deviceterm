...
//
// Role protocol: issue a live orchestration grant for a tab's sessions.
//
...
// an orchestrator tab has been created AND terminal-bound, the terminal-pane
// container calls this so the daemon marks the session orchestration-authorized:
...
// it, those verbs stay refused even in an orchestrator tab.
//
// `orchestrator.grant` is `.validatedGUI`-scoped, so no cap rides on the wire:
...
@MainActor
protocol OrchestratorGranting: AnyObject {
    /// Grant a live orchestration lease to `sessionIds`. The client stamps a
