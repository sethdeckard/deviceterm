...
/// can be protected, and both then show. The order is stated once here and
/// pinned by a test rather than left to whatever order the strip's rendering
/// code happens to mount them in.
enum TabMarkerDecision {
    ...
    /// marker on the inside means a protection toggle never shifts the other.
    ///
