...
enum PaneEvent: Sendable {
    ...
    /// keeps its previous surface) and always nil over UDS. A leased frame
    /// carries a leased `SurfaceLease` (use-count bumped, released on ARC
    /// deinit → the daemon frees the slot); an unleased frame (any frame when
    /// `DEVICETERM_SURFACE_LEASES` is off) carries a `SurfaceLease` with no
    /// use-count and no ack.
