...
description: >-
  ...
  hardware buttons, text, rotation, accessibility reads, and waits on observable
  device state against a simulator or connected device. Use when asked to
  exercise or verify device-control behaviour, coordinate mapping in any
  orientation, or `ax tree`/`point`/`sweep` output. Pairs an input verb with an
  accessibility read-back, since a receipt reports dispatch and never what the
  app did. For deviceterm's own AppKit chrome (windows, tabs, panes, status
  item, prompts), use `deviceterm-e2e` instead.
---
