# `pane send-input` Manual Checklist
...
`TerminalPaneViewController.sendInput`, or the `pane send-input`
dispatcher.
...
| # | Action | Expected |
|---|--------|----------|
| 1.1 | From the driver: `deviceterm pane send-input <pane> -- 'echo hello\n'` | In the recorded pane the command appears **and executes**: `hello` on its own line, then a fresh prompt. It must not sit unexecuted at the prompt. |
...
| 1.4 | Send a bare newline: `-- '\n'`. | The recorded pane advances to a new blank prompt line. |
...
that reproduce through `deviceterm pane send-input` alone are injection;
losses that appear only under `demo-present.sh` are the driver.

| # | Action | Expected |
|---|--------|----------|
| 4.1 | Fire three paced sends back-to-back without waiting, each `-- 'echo deviceterm\n'` at `--type-delay 45`. | All three run. Every output line reads `deviceterm`; no leading character disappears and no `command not found` appears. |
| 4.2 | Read the recorded pane's scrollback for the whole run. | No missing or duplicated characters anywhere, at any command boundary. |

## 5. End-to-end through the demo driver

| # | Action | Expected |
|---|--------|----------|
| 5.1 | From the driver, `scripts/demo-present.sh scripts/demo-example.txt --target <pane>`, advancing each step. | Each keypress types **and runs** one command. One prompt per command; commands never stack up in a single buffer. |
| 5.2 | Re-run with `--no-settle` and advance a step early, while a previous paced command is still typing. | The next command queues behind it and types out after. Order preserved, no interleaving, no lost characters. **`--no-settle` is required**: by default the driver waits for the recorded pane's viewport to hold still before offering a step, so it will not let you advance mid-typing. Two back-to-back `deviceterm pane send-input` calls by hand test the same thing. |
| 5.3 | Run without `--no-settle` and try to advance during the boot step. | The driver refuses to offer the next step until the recorded pane viewport goes quiet, printing `waiting for the recorded pane viewport to settle…`. |
