...
// The GUI half of the orchestration-grant lifecycle: once an orchestrator tab's
...
//
// Paired with the daemon half (`OrchestratorGrantUDSScopeTests`, a granted UDS
// session reaches `tab.sendInput`/`tab.capture`), this is the end-to-end
// "open orchestrator tab → grant issued → CLI reaches the verbs" chain, split
// at the process boundary a unit test can't cross.

@MainActor
private func makeCoordinator(_ fake: FakeDaemonClient) -> OrchestratorGrantCoordinator {
    // No-delay sleep so retries run without real time.
    OrchestratorGrantCoordinator(client: fake, sleep: { _ in true })
}
...
@MainActor
private func drain(_ coord: OrchestratorGrantCoordinator) async {
...
@MainActor
struct OrchestratorGrantCoordinatorTests {
    @Test
    func orchestratorTabIssuesGrantForBoundSession() async {
        let fake = FakeDaemonClient()
        let coord = makeCoordinator(fake)
        let sid = UUID()
        coord.sessionBound(role: .orchestrator, sessionId: sid.uuidString)
        await drain(coord)
        #expect(fake.grantOrchestratorCalls.count == 1)
        #expect(fake.grantOrchestratorCalls.first?.sessionIds == [sid])
    }
    ...
    func agentTabIssuesNoGrant() async {
        ...
        await drain(coord)
        #expect(fake.grantOrchestratorCalls.isEmpty)
    }
    ...
    func malformedSessionIdIssuesNoGrant() async {
        let fake = FakeDaemonClient()
        let coord = makeCoordinator(fake)
        coord.sessionBound(role: .orchestrator, sessionId: "not-a-uuid")
        await drain(coord)
        #expect(fake.grantOrchestratorCalls.isEmpty)
    }
    ...
    func transientFailureRetriesWithFreshRevisionsUntilApplied() async {
        ...
        let fake = FakeDaemonClient()
        fake.grantOrchestratorFailures = [
            DaemonClientError.transport("blip"),
            DaemonClientError.daemon(code: -32_002, message: "notReady")
        ]
        let coord = makeCoordinator(fake)
        coord.sessionBound(role: .orchestrator, sessionId: UUID().uuidString)
        await drain(coord)
        #expect(fake.grantOrchestratorCalls.count == 3)
        let revisions = fake.grantOrchestratorCalls.map(\.revision)
        #expect(revisions == revisions.sorted() && Set(revisions).count == 3)
    }

    @Test
    func terminalErrorStopsRetrying() async {
        // A dead session (`invalidParams`) or a stable role_violation is
    ...
    func appliedFalseStopsRetrying() async {
        ...
        let fake = FakeDaemonClient()
        fake.grantOrchestratorApplied = [false]
        let coord = makeCoordinator(fake)
        coord.sessionBound(role: .orchestrator, sessionId: UUID().uuidString)
        await drain(coord)
        #expect(fake.grantOrchestratorCalls.count == 1)
    ...
    func reconnectRebindReissuesWithDominatingRevision() async {
        ...
        coord.sessionBound(role: .orchestrator, sessionId: sid.uuidString)
        await drain(coord)
        #expect(fake.grantOrchestratorCalls.count == 2)
        let revisions = fake.grantOrchestratorCalls.map(\.revision)
    ...
    func sessionRemovedCancelsParkedRetryBeforeItRegrants() async {
        ...
        let gate = SleepGate()
        let fake = FakeDaemonClient()
        fake.grantOrchestratorFailures = [DaemonClientError.transport("blip")]
        let coord = OrchestratorGrantCoordinator(client: fake, sleep: { await gate.sleep($0) })
        let sid = UUID()
        coord.sessionBound(role: .orchestrator, sessionId: sid.uuidString)
        await yieldUntil { gate.parkedCount == 1 }
        #expect(fake.grantOrchestratorCalls.count == 1)  // threw once, now parked
        ...
        #expect(fake.grantOrchestratorCalls.count == 1)  // no regrant after removal
    }
    ...
    func cancelAllCancelsEveryParkedRetry() async {
        ...
        #expect(fake.grantOrchestratorCalls.count == 2)  // both threw once, parked

        coord.cancelAll()
        #expect(coord.pendingSessionsForTesting.isEmpty)
        gate.releaseAll(true)
        for _ in 0..<200 { await Task.yield() }
        #expect(fake.grantOrchestratorCalls.count == 2)  // neither retried after cancelAll
    }
