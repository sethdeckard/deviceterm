...
// set the daemon ships. The orchestrator surface in `allowedMethods`
// follows the session's live orchestration GRANT, not its role (see
// the XPC grant-matrix tests in `OrchestratorGrantScopeTests`); these
...
//   2. valid orchestrator creds → role: .orchestrator, no grant → same subset
...
@Test
func capabilitiesWithOrchestratorCredsReturnsOrchestratorRole() async throws {
    let (registry, manager) = makeRegistry()
    ...
    let handler = try capabilitiesHandler(registry)
    let response = try await invokeCapabilities(handler, session: state)
    #expect(response.role == .orchestrator)
    // The role is reported, but it grants nothing: an ungranted UDS session
    // omits orchestrator capabilities. The grant-based advertising matrix is
    // exercised over XPC in `OrchestratorGrantScopeTests`.
    ...
    // CLI caller (even an orchestrator) never sees it advertised.
    #expect(!response.allowedMethods.contains(RPCMethod.sessionSetPrivateBatch.rawValue))
}

@Test
func capabilitiesWithAgentCredsOmitsOrchestratorMethods() async throws {
    // The other half of the pairing above: an ungranted session doesn't
    // see the orchestrator surface. An agent without a live grant omits
    // orchestrator capabilities; a granted agent sees them over UDS or XPC
    // (per `OrchestratorGrantScopeTests`).
    let (registry, manager) = makeRegistry()
...
@Test
func capabilitiesOverUDSOmitsOrchestratorMethods() async throws {
    ...
    // creds), so authenticate as the orchestrator session first.
